...
//generate_currencyxml($xmltime, $_GET["from"], $fromrate[0]["currency"], $fromcountry, $_GET["amnt"], $_GET["to"], $torate[0]["currency"], $tocountry, $conversion, $_GET["format"]);
...
/*
function generate_currencyxml($atfun, $fromcodefun, $fromcurrfun, $fromcountrfun, $fromamntfun, $tocodefun, $tocurrfun, $tocountrfun, $toamntfun, $format='xml') {
	...
	if ($format=='json') {
		...
		$xml .= '<at>'. $atfun . '</at>';
		$xml .= '<from>';
		$xml .= '<code>'. $fromcodefun . '</code>';
		$xml .= '<curr>'. $fromcurrfun . '</curr>';
		$xml .= '<loc>'. $fromcountrfun . '</loc>';
		$xml .= '<amnt>'. $fromamntfun . '</amnt>';
		$xml .= '</from>';
		$xml .= '<to>';
		$xml .= '<code>'. $tocodefun . '</code>';
		$xml .= '<curr>'. $tocurrfun . '</curr>';
		$xml .= '<loc>'. $tocountrfun . '</loc>';
		$xml .= '<amnt>' . $toamntfun . '</amnt>';
		$xml .= '</to>';
		$xml .= '</conv>';
